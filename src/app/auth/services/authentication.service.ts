...
export class AuthenticationService {
    ...
    const processedUserDTOObservable: Observable<UserDTO> = userDTOObservable.pipe(
      map((userDTO: UserDTO) => {
        if (redirectToLoginUri) {
          this.router.navigate([APP_UI_ROUTES.LOGIN], { queryParams: { [QueryParamUIKey.REGISTRATION_SUCCESSFUL]: "Registration Successful, Please check your email for verification." }, replaceUrl: true });
        }
        return userDTO;
      })
    );
  ...
  public verifyEmailVerificationRequest(verifyEmailContext: VerifyEmailContext, redirectToLoginUri?: boolean): Observable<GenericResponse<boolean>> {
    ...
    const processedVerificationCodeObservable: Observable<GenericResponse<boolean>> = checkVerificationCodeResponse.pipe(
      map((verificationResponse: GenericResponse<boolean>) => {
        if (redirectToLoginUri) {
          this.router.navigate([APP_UI_ROUTES.LOGIN], { queryParams: { [QueryParamUIKey.EMAIL_VERIFICATION_SUCCESSFUL]: "Email Verification Successful" }, replaceUrl: true });
        }
    ...
    const processedFPVerificationResponseObs: Observable<GenericResponse<boolean>> = fpVerificationResponseObs.pipe(
      map((fpVerificationResponse: GenericResponse<boolean>) => {
        if (redirectToLoginUri) {
          ...
          this.router.navigate([APP_UI_ROUTES.LOGIN], { queryParams: { [QueryParamUIKey.PASSWORD_RESET_SUCCESSFUL]: "Password Reset Successful. Please Login" }, replaceUrl: true });
        }
  ...
  public logout(shouldRedirect: boolean = true, defaultRedirectUri: string = APP_UI_ROUTES.LOGIN): Observable<boolean> {
